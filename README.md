...
默认告警阈值 **2,000 ms**，按 **1 分钟平均值**判断，每分钟评估一次。指标是 Foundry 原生 TTLT，不是客户端端到端耗时，也不是首 token 时延。
...
- [部署步骤](docs/deployment.md)：外部资源前置、离线生成、控制器部署、授权、策略安装和启用。
...
**部署不是运行一次脚本就立即接管流量。** 先离线生成并审阅，部署控制资源，再授予精确范围权限，显式安装策略，最后启用告警。已有 API 策略会被替换，必须先保存可恢复的版本。
